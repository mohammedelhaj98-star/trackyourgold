...
## Malabar scraping behavior
- Scraper now uses a **Malabar-first source chain**: `MALABAR_URL` then `MALABAR_STORES_URL`.
- Parser supports extraction from inline JSON script payloads and visible text/table patterns.
- Ingestion response now includes `sourceUrl` and `pageTimestamp` when detected.
